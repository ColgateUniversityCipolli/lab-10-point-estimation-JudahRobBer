...
leg_spread = bind_rows(
  dat.pdc.cleaned |> select(player = Winner, legs_won = WinnerLegs, legs_lost = LoserLegs),
  dat.pdc.cleaned |> select(player = Loser,  legs_won = LoserLegs,  legs_lost = WinnerLegs)
) |>
  group_by(player) |>
  summarise(leg_spread = sum(legs_won) - sum(legs_lost))


leg_spread |>
  arrange(leg_spread) |>
  mutate(player = factor(player, levels = player)) |> 
  ggplot(aes(x = leg_spread, y = player)) +
  geom_col() +
  geom_vline(xintercept = 0, linewidth = 0.8) +
  labs(x = "Leg Spread", y = NULL, title = "Leg Spread by Player") +
  theme_bw() +
...
find_mle = function(df,player) {
  legs = bind_rows(
    dat.pdc.cleaned |> filter(Winner == player) |> select(legs = WinnerLegs),
    dat.pdc.cleaned |> filter(Loser == player)  |> select(legs = LoserLegs)
  )$legs
  
  maxll = optim(par=c(.5,1),
          fn = ll.altbiom,
          data = legs,
          neg=T)
  maxll
...
for (player in players) {
  mle = find_mle(dat.pdc.cleaned, player)
...
#| tbl-cap: "MLE Estimation for Altbinom by Player"

df.altbinom.mle |> knitr::kable(digits=2)
```
